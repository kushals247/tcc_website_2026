...
$image = get_field('insp_hero_image');
$eyebrow = get_field('insp_hero_eyebrow');
$headline = get_field('insp_hero_headline');
$subline = get_field('insp_hero_subline');

if (!$image && !$headline) return;

$image_url = is_array($image) ? ($image['url'] ?? '') : (is_string($image) ? $image : '');
?>
<section class="tc-insp-hero relative h-[40vh] md:h-[50vh] w-full overflow-hidden flex items-end"
