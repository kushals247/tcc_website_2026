...
$headline = get_field('contact_hero_headline') ?: get_the_title();
$subline = get_field('contact_hero_subline');

if (!$image && !$headline) return;

$image_url = is_array($image) ? $image['url'] : (is_string($image) ? $image : '');
?>
<section class="tc-contact-hero relative h-[30vh] md:h-[40vh] w-full overflow-hidden flex items-end"
    ...
    <div class="relative z-10 max-w-5xl mx-auto px-6 pb-10 w-full">
        <?php if ($headline): ?>
